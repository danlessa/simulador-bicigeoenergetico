...
const STRINGS = {
  ...
  "maxseg.nseg.title":   { pt: "Segmentos consecutivos não-sobrepostos: depois de cada segmento encontrado, a densidade é zerada num tubo de ~2 células ao redor dele e a busca roda de novo no que sobrou. As células continuam transitáveis — um segmento posterior pode CRUZAR um anterior perpendicular (só não coleta nada ali), mas não pode repassar o mesmo corredor nem a pista vizinha.", en: "Consecutive non-overlapping segments: after each found segment, the density is zeroed in a ~2-cell tube around it and the search re-runs on what is left. Cells stay traversable — a later segment may CROSS an earlier one perpendicular (it just collects nothing there), but cannot re-harvest the same corridor or its adjacent lane." },
